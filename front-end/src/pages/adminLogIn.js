    ...
    try {
        if (checkFormInput()) {
          ...
          if (response.data.authenticated) {
              ...
              setmissingFields("The email or the password is incorrect!  ");
          }
        }
      } catch(error) {
            ...
            <form onSubmit={handleSubmit} className="w-full">
              ...
              <div className="relative mt-10">
                <input
                  type="submit"
                  value="Log In"
                  className=" text-center  text-white bg-primary font-semibold bg-purpleLight border-0 p-4 focus:outline-none  rounded-md text-sm hover:cursor-pointer"
                />
              </div>
            </form>
          </div>
